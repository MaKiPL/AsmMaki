...
void RefreshRegisters(WINDOW * regWin, WINDOW * segWin)
{
	...
	wclear(regWin);
	wborder(regWin, (char)221, (char)221, (char)220, (char)223, (char)222, (char)219, (char)220, (char)220);
	sprintf(EAXbuffer, "EAX\t%04X %02X %02X\tSF %00X (Sign)",(EAX>>16)&0xFFFF, ((EAX>>8)&0xFF), EAX&0xFF, SF&0x1);
	mvwaddstr(regWin, 1, 2, EAXbuffer);
	sprintf(EAXbuffer, "EBX\t%04X %02X %02X\tZF %00X (Zero)", (EBX >> 16) & 0xFFFF, ((EBX >> 8) & 0xFF), EBX & 0xFF, ZF & 0x1);
	mvwaddstr(regWin, 2, 2, EAXbuffer);
	sprintf(EAXbuffer, "ECX\t%04X %02X %02X\tOF %00X (Overfl.)", (ECX >> 16) & 0xFFFF, ((ECX >> 8) & 0xFF), ECX & 0xFF, OF & 0x1);
	mvwaddstr(regWin, 3, 2, EAXbuffer);
	sprintf(EAXbuffer, "EDX\t%04X %02X %02X\tCF %00X (Carry)", (EDX >> 16) & 0xFFFF, ((EDX >> 8) & 0xFF), EDX & 0xFF, CF & 0x1);
	mvwaddstr(regWin, 4, 2, EAXbuffer);
	sprintf(EAXbuffer, "ESI\t%04X %02X %02X\tPF %00X (Parity)", (ESI >> 16) & 0xFFFF, ((ESI >> 8) & 0xFF), ESI & 0xFF, PF & 0x1);
	mvwaddstr(regWin, 5, 2, EAXbuffer);
	sprintf(EAXbuffer, "EDI\t%04X %02X %02X\tAF %00X (Auxili.)", (EDI >> 16) & 0xFFFF, ((EDI >> 8) & 0xFF), EDI & 0xFF, AF & 0x1);
	mvwaddstr(regWin, 6, 2, EAXbuffer);
	sprintf(EAXbuffer, "EBP\t%04X %02X %02X", (EBP >> 16) & 0xFFFF, ((EBP >> 8) & 0xFF), EBP & 0xFF);
	mvwaddstr(regWin, 7, 2, EAXbuffer);
	sprintf(EAXbuffer, "EIP\t%04X %02X %02X", (EIP >> 16) & 0xFFFF, ((EIP >> 8) & 0xFF), EIP & 0xFF);
	mvwaddstr(regWin, 8, 2, EAXbuffer);
	sprintf(EAXbuffer, "ESP\t%04X %02X %02X", (ESP >> 16) & 0xFFFF, ((ESP >> 8) & 0xFF), ESP & 0xFF);
